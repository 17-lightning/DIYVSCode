...
function DIY_book(context) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            target = target.replace(new RegExp("/", "g"), "--");
            target = target + ".md";
            if (fs.existsSync(path.join(yield get_DIY_library(), target))) {
                // 目标MD存在，就会直接跳转到她
...
function DIY_log_in_relation(document, type) {
    ...
    try {
        ...
        while (line != ("## " + type) && (lineid + 2 < document.lineCount)) {
            lineid = lineid + 1;
            line = (_a = document.lineAt(lineid)) === null || _a === void 0 ? void 0 : _a.text;
        }
...
function DIY_show_function_document(context, target) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            let lineid = 0;
            let document = yield vscode.workspace.openTextDocument(path.join(yield get_DIY_library(), target));
            ...
            const panel = vscode.window.createWebviewPanel('testWebView', document.lineAt(0).text.substring(2), vscode.ViewColumn.Two, {
                enableScripts: true,
...
function DIY_create_current_function_document(context) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
...
function convert_relation(input) {
    if (input == 'childList') {
        return 'parentList';
    }
    if (input == 'parentList') {
...
function DIY_book_add_relation(me, key, value, note, type, update) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            Box.debug("[ltn] DIY_book_add_relation 开始");
            let filepath = path.join(yield get_DIY_library(), me + ".md");
            // 从这里开始改用fs吧
...
function DIY_book_relation_edit(context, panel, message) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            let filepath = path.join(yield get_DIY_library(), message.me + ".md");
            ...
            while (lineid < document.lineCount) {
                ...
                if (line == "## " + target) {
                    ...
                    if (flag == 1) {
                        if (array[1] == "add") {
                            // 检查value是否有效
                            if (fs.existsSync(path.join(yield get_DIY_library(), message.value + ".md"))) {
...
function DIY_goto_target(filepath, target) {
    return __awaiter(this, void 0, void 0, function* () {
        let document = yield vscode.workspace.openTextDocument(filepath);
        let lineid = 0;
        let line;
 ...
 */
function DIY_book_html_handler(context, panel, message) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            Box.sep_debug();
