...
export interface function_t {
    name : string;
    lineid : number;
};


export interface relation_t {
    key: string;
    value: string;
    note: string;
};
...
export class FileParser {
    ...
    // 用这个函数来创建对象
    static create(filepath: string): FileParser | null {
        try {
    ...
    set_position_invalid(position : Position) {
        position.line = -1;
        position.index = -1;
    }

    // 跳转下一个位置，修改会在当前position(入参)中发生
    get_next_position(position : Position) {
    ...
    get_former_position(position : Position) {
        if (position.index > 0) {
            position.index = position.index - 1;
    ...
    try_get_next_position(position : Position) : Position {
        var pos2 = {line: position.line, index: position.index};
        this.get_next_position(pos2);
        return pos2;
    }

    try_get_former_position(position : Position) : Position {
        var pos2 = {line: position.line, index: position.index};
    ...
    // 获取当前位置的值，位置不正确时返回""
    get_current(position : Position) : string {
    ...
    // 简单版判断当前位置是否有效
    is_valid_position_simple(position : Position) : boolean {
        if (position.line < 0 || position.index < 0) {
            return false;
        }
        return true;
    ...
    get_trans_name(input : string) : string {
...
export class CFileParser extends FileParser {
    ...
    static is_symbol(target : string) : boolean {
        if (target.length == 0) {
            return false;
        }
    ...
    get_next_symbol(position : Position) : string {
        var result = "";
    ...
    get_symbol_of_line(line : number) : string[] {
    ...
    // 获取当前函数在哪一行
    get_function_line(target : string) : number {
    ...
    getChildFunction(target : string, searchPath : string[] | undefined) : Array<relation_t> {
        var temp : any;
        var result : Array<relation_t> = [];
        // 先刷一把函数列表
        if (this.function_list.length == 0) {
            this.flushFunctionList();
        }
...
export function turnTransAddrToComplete(input : string) {
...
export function get_next_position(content : string[], position : Position) : Position {
...
export function get_former_position(content : string[], position : Position) : Position {
    if (position.index > 0) {
        return {line : position.line, index : position.index - 1};
    } else if (position.line > 0) {
...
export function get_next_element(content : string[], position : Position) {
    var result = "";
...
// 在目标文件中查找函数
async function find_function_definition(filepath : string, funcname : string) {
    try {
        ...
        TBox.debug((error as Error).toString());
    }
}
