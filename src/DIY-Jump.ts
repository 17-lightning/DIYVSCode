...
export async function DIY_Jump() {
    ...
    if (workspace != undefined) {
        ...
        if (await DIY_Jump_inner(workpath, keyword)) {
            Box.debug("使用当前工作区下的跳转规则跳转成功");
            return;
        }
    }
    // 最后使用默认跳转规则
    workpath = vscode.workspace.getConfiguration().get<string>('diyvscode.DiyJumpRules');
 ...
 * @returns 是否完成跳转
 */
async function DIY_Jump_inner(configfile : string, key : string) : Promise<boolean> {
    if (!fs.existsSync(configfile)) {
        return false;
    }
    let document = await vscode.workspace.openTextDocument(vscode.Uri.file(configfile));
    let lineid = 0;
    let line : string;
    ...
    for (lineid = 0; lineid < document.lineCount; lineid++) {
        ...
        if (targetfile[0] == '@') { // @开头的路径表示以当前工作区(仅支持首个工作区)为起始路径
            ...
            let custompath : string | undefined = vscode.workspace.getConfiguration().get('diyvscode.diyjumptoppath');
        ...
        let target : vscode.TextDocument = await vscode.workspace.openTextDocument(targetfile);
        let targetloc : string = line.split('|')[2]; // 配置的第三项可以帮助你找到对应行
